...
c integration of ODE's inside FINDX
      ...
      REAL(8) t, dt, tFinal, pMin, temp
      
      CHARACTER(8048) string
      CHARACTER(32) sTmp
      CHARACTER flag
      
      REAL(8), ALLOCATABLE :: Qi(:), Qf(:), Pi(:), Pf(:), X(:), Xo(:), 
     2   f(:,:)
     ...
     2      ' should match with nNeumannSrfs defined in USER.f'
      ...
      IF (nDirichletSrfs .GT. 0) THEN
         IF (qCorr .OR. pCorr) THEN
            PRINT *, 'ERROR: You should only use P/Q correction when',
     2         ' all the surfaces are Neumann surfaces'
            STOP
         END IF
      END IF
      DO i=1, nDirichletSrfs
         IF (srfToXdPtr(i).LT.1 .OR. srfToXdPtr(i).GT.nUnknowns) THEN
            PRINT *, "ERROR: srfToXdPtr(", i, ") is unexpected"
            STOP
         END IF
      END DO
      DO i=1, nNeumannSrfs
         IF (srfToXPtr(i).LT.1 .OR. srfToXPtr(i).GT.nUnknowns) THEN
            PRINT *, "ERROR: srfToXPtr(", i, ") is unexpected"
            STOP
         END IF
      END DO
...
c Setting up the system of equations
      ...
      DO n=1, nTimeStep
         ...
         CALL FINDF (t, Xo, f(:,1), QNeumann(:,1), 
     2      PDirichlet(:,1))
         X = Xo + dt*f(:,1)/3D0
...
c Time to write the results
      ...
      IF (pCorr .AND. flag.NE.'D') THEN
         pMin = X(srfToXPtr(1)) + offset(srfToXPtr(1))
         DO i=2, nNeumannSrfs
            IF (X(srfToXPtr(i)) .LT. pMin) THEN
               pMin = X(srfToXPtr(i)) + offset(srfToXPtr(i))
            END IF
         END DO
      ELSE
         pMin = 0D0
...
c Writing nDirichlet flowrates here
      OPEN (1, FILE='GenBC.int', STATUS='OLD', FORM='UNFORMATTED')
      DO i=1, nDirichletSrfs
         WRITE (1) (X(srfToXdPtr(i)) + offset(srfToXdPtr(i)))*qConv
c         WRITE (*,*) (X(srfToXdPtr(i)))
      END DO
